# -*- coding: utf-8 -*-
"""
Created on Thu Dec 29 14:18:38 2022
...
def load_floris():
    ...
    fi.reinitialize(layout_x=[0,-421.81,-573.73,-371.54],layout_y=[0,393.34,1199.07,834.49])
    ...
    visualize_cut_plane(horizontal_plane, ax=None, title="La Haute Borne Wind Turbine Locations")
    plt.savefig("outputs/laHauteBorneFrance/windFarm.pdf")
    plt.show()
...
def load_windrose():
    fn = "inputs/laHauteBorneFrance/windRose.csv"
    df = pd.read_csv(fn)
...
if __name__ == "__main__":
    ...
    for ii, idx in enumerate(df_windrose.index):
        ...
        if wind_speed > 12.0 or wind_speed < 2.0:
    ...
    plt.savefig("outputs/laHauteBorneFrance/windDirectionAEPuplift.pdf")
    ...
    plt.savefig("outputs/laHauteBorneFrance/windSpeedAEPuplift.pdf")
    ...
    for ti in range(nturbs):
        ...
        ax.plot(
            ...
            color="dodgerblue",
            markersize=3,
            label="2 m/s < Wind Speed < 4 m/s $\\bf{and}$ 10 m/s < Wind Speed < 12 m/s",
        )
        ax.plot(
            df_opt["wind_direction"],
            ...
            color="grey",
            markersize=3,
            label="Wind Speed < 2 m/s $\\bf{and}$ Wind Speed > 12 m/s",
        )
        ...
        plt.tight_layout()
        plt.savefig("outputs/laHauteBorneFrance/turbine{:d}.pdf".format(ti))
