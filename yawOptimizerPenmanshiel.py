# -*- coding: utf-8 -*-
"""
Created on Fri Dec 30 13:06:34 2022
...
def load_floris():
    ...
    fi.reinitialize(layout_x=[0, -277.18, 382.43, 88.02, -172.38, -417.99, 601.6, 276.51, 58.9, -183.38, 689.17, 502.79, 283.18, -4.33],
                    layout_y=[0, 569.14, 411.1, 891.56, 1380.57, 1837.46, 1008.59, 1620.74, 2089.86, 2669.34, 1711.87, 2278.57, 2749.47, 3229.7])
    ...
    visualize_cut_plane(horizontal_plane, ax=None, title="Penmanshiel Wind Turbine Locations")
    plt.savefig("outputs/penmanshielUK/windFarm.pdf")
    plt.show()
...
def load_windrose():
    fn = "inputs/penmanshielUK/windRose.csv"
    df = pd.read_csv(fn)
...
if __name__ == "__main__":
    ...
    for ii, idx in enumerate(df_windrose.index):
        ...
        if wind_speed > 12.0 or wind_speed <= 3.0:
    ...
    plt.savefig("outputs/penmanshielUK/windDirectionAEPuplift.pdf")
    ...
    plt.savefig("outputs/penmanshielUK/windSpeedAEPuplift.pdf")
    ...
    for ti in range(nturbs):
        ...
        ax.plot(
            ...
            color="dodgerblue",
            markersize=3,
            label="3 m/s < Wind Speed < 4 m/s $\\bf{and}$ 10 m/s < Wind Speed < 12 m/s",
        )
        ax.plot(
            df_opt["wind_direction"],
            ...
            color="grey",
            markersize=3,
            label="Wind Speed < 3 m/s $\\bf{and}$ Wind Speed > 12 m/s",
        )
        ...
        plt.tight_layout()
        plt.savefig("outputs/penmanshielUK/turbine{:d}.pdf".format(ti))
